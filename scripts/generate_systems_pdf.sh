...
This module compiles the Systems documentation where each subsystem lives in its
...
embedded About PDF renderer (cover page, TOC, headings, tables, lists)
and just changes how markdown sources are collected.

Usage via CLI wrapper:
  ./scripts/generate_systems_pdf.sh [--input-dir docs/03-systems] [--output <path>]
...
def _load_about_module():
    """Load the embedded About PDF renderer."""
    ...
    exec(ABOUT_MODULE_SOURCE, module.__dict__)
