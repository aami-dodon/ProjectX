...
- Keep `DEMO_FLAG=true` when you need the full demo workspace. On server startup, `server/src/modules/demo/demo.seed.js` wipes prior demo-tagged rows and repopulates every Prisma model with coherent records so the API and dashboards have data. Set the flag to `false` for clean databases or production environments.
