...
6. Confirm API connectivity at `http://localhost:${SERVER_PORT}/api/v1/health` (defaults to `http://localhost:6006/api/v1/health`).
...
| (no env) | API base path is fixed to `/api/v1`. |
...
> Note: `VITE_API_URL` should be the server origin only (e.g., `http://localhost:6006`) — do not include the `/api/v1` path. Routes in the frontend already use `/api/v1/...`.
