...
### Relationships to Frameworks
Framework mappings live in a `control_framework_links` join table linking `control_id` to `framework_id` and `requirement_id`. A control can map to multiple frameworks and requirements, and mappings include the following metadata:
...
### Linkage to Remediation Tasks and Dashboards
